  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[var(--color-warm-text)]">Products linked in videos</h1>
          <p className="text-muted-foreground">Manage material combo links and measure add-to-cart intent from lessons.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="rounded-full bg-[var(--color-success)] hover:bg-[var(--color-warm-accent-hover)]"><Plus className="mr-2 h-4 w-4" />Add product link</Button></DialogTrigger>
          <DialogContent>
            ...
            <DialogFooter><Button onClick={() => setOpen(false)} className="bg-[var(--color-success)] hover:bg-[var(--color-warm-accent-hover)]">Add link</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
